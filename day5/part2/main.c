...
int main()
{
    ...
    while (fgets(buffer, 128, file) != NULL)
    {
        ...
        else if (line_count >= 10)
        {
            ...
            int from_free = get_next_free_id(stacks[from_col - 1]);
            for (int i = 0; i < crate_count; ++i)
            {
                int from_id = from_free - crate_count + i;
                char from_crate = stacks[from_col - 1][from_id];
                int to_free = get_next_free_id(stacks[to_col - 1]);
                stacks[to_col - 1][to_free] = from_crate;
                stacks[from_col - 1][from_id] = 0;
            }
        }
